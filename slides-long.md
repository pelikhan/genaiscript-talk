...
<!-- 
Now when you need agentic capabilities, you can create tools to allow AI models to manipulate the environment through code, and agents that can wrap a prompt and a set of tools to perform specific tasks.

Note that creating an agent also creates a tool prefixed by `agent_`, and this is the how you orchestrate agents in GenAIScript. It doesn't provide any agentic workflow capabilities, so orchestration relies on tools.
...
Also, a quick note about tools vs system here: system config allows to load a system-defined script, that can contains multiple agents and tools.
-->
...
<!-- 
You can also finet-tune the behavior of your script using metadata.
...
<!-- 
You've also seen that you the GenAIScript CLI can run as an MCP server, allowing you to expose your scripts as MCP tools to use with other clients, like VS Code or Claude.
